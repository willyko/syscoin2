...
UniValue aliasfeedbackfilter(const UniValue& params, bool fHelp) {
	if (fHelp || params.size() > 2)
		throw runtime_error(
				"aliasfeedbackfilter [[[[[regexp]] from=0]]]\n"
						"scan and filter feedback for aliases used in escrow\n"
						"[regexp] : apply [regexp] on aliases or escrow ID, empty means all alias feedbacks\n"
						"[from] : show results from this escrow GUID [from], 0 means first.\n");
    ...
    if (!pescrowdb->ScanEscrowFeedbacks(vchEscrow, strRegexp, 25, escrowScan))
        throw runtime_error("scan failed");

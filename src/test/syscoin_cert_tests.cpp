...
BOOST_AUTO_TEST_CASE (generate_big_certdata)
{
	...
	string guid = CertNew("node1", "jag", gooddata);
	// unencrypted we are allowed up to 1108 bytes
	BOOST_CHECK_NO_THROW(CallRPC("node1", "certnew jag1 " + baddata + " 0"));
	// but unencrypted 1024 bytes should cause us to trip 1108 bytes once encrypted
	BOOST_CHECK_THROW(CallRPC("node1", "certnew jag1 " + baddata + " 1"), runtime_error);
...
BOOST_AUTO_TEST_CASE (generate_big_certtitle)
{
	...
	CertNew("node1", goodtitle, "a");
	BOOST_CHECK_THROW(CallRPC("node1", "certnew " + badtitle + " 3d"), runtime_error);
}
...
BOOST_AUTO_TEST_CASE (generate_certtransfer)
{
	...
	certtitle = "certtitle";
	certdata = "certdata";
	guid = CertNew("node1", certtitle, certdata);
	// private cert
	pvtguid = CertNew("node1", certtitle, certdata);
